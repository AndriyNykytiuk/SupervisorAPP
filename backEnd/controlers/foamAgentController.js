import { FoamAgent, Brigade } from '../models/index.js'
...
export const getAll = async (req, res, next) => {
    try {
        ...
        if (!req.scope) {
            const items = await FoamAgent.findAll({ include: [Brigade] })
        ...
        if (req.scope.detachmentId && !req.scope.brigadeId) {
            ...
            const items = await FoamAgent.findAll({
...
export const getById = async (req, res, next) => {
    try {
        const item = await FoamAgent.findByPk(req.params.id, { include: [Brigade] })
...
// Отримання всіх записів для однієї бригади
export const getByBrigade = async (req, res, next) => {
    try {
        ...
        const items = await FoamAgent.findAll({ where: { brigadeId } })
        res.json(items)
    } catch (err) {
        next(err)
    }
...
export const create = async (req, res, next) => {
    try {
        ...
        const newItem = await FoamAgent.create({
            ...req.body,
...
export const update = async (req, res, next) => {
    try {
        const item = await FoamAgent.findByPk(req.params.id)
        if (!item) return res.status(404).json({ error: 'Item not found' })
