import { ExtenguisDocumentLink, Brigade } from '../models/index.js'

// Отримання всіх записів з урахуванням ролі
export const getAll = async (req, res, next) => {
    try {
        if (!req.scope) {
            const items = await ExtenguisDocumentLink.findAll({ include: [Brigade] })
        ...
        if (req.scope.detachmentId && !req.scope.brigadeId) {
            ...
            const items = await ExtenguisDocumentLink.findAll({
...
export const getById = async (req, res, next) => {
    try {
        const item = await ExtenguisDocumentLink.findByPk(req.params.id, { include: [Brigade] })
...
export const getByBrigade = async (req, res, next) => {
    try {
        ...
        const items = await ExtenguisDocumentLink.findAll({ where: { brigadeId } })
        res.json(items)
    } catch (err) {
        next(err)
    }
...
export const create = async (req, res, next) => {
    try {
        ...
        const newItem = await ExtenguisDocumentLink.create({
...
export const update = async (req, res, next) => {
    try {
        const item = await ExtenguisDocumentLink.findByPk(req.params.id)
        if (!item) return res.status(404).json({ error: 'Item not found' })
