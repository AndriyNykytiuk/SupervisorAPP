import { SwimTools, Brigade } from '../models/index.js'
...
export const getAll = async (req, res, next) => {
    try {
        ...
        if (!req.scope) {
            const items = await SwimTools.findAll({ include: [Brigade] })
        ...
        if (req.scope.detachmentId && !req.scope.brigadeId) {
            ...
            const items = await SwimTools.findAll({
...
export const getById = async (req, res, next) => {
    try {
        const item = await SwimTools.findByPk(req.params.id, { include: [Brigade] })
...
export const getByBrigade = async (req, res, next) => {
    try {
        ...
        const items = await SwimTools.findAll({ where: { brigadeId } })
        res.json(items)
    } catch (err) {
        next(err)
    }
...
export const create = async (req, res, next) => {
    try {
        ...
        const newItem = await SwimTools.create({
            ...req.body,
...
export const update = async (req, res, next) => {
    try {
        const item = await SwimTools.findByPk(req.params.id)
        if (!item) return res.status(404).json({ error: 'Item not found' })
